...
def setup(bot: commands.Bot):
    ...
    # /랭킹
    # ────────────────────────────────────────────
    @tree.command(name="랭킹", description="이번 시즌 수익률 랭킹을 조회합니다")
    async def cmd_ranking(interaction: discord.Interaction):
        await interaction.response.defer()
        users = db.get_all_users(SEASON)
        if not users:
            await interaction.followup.send("아직 참가자가 없어요. `/잔고` 를 먼저 실행해 등록하세요!")
            return

        scores = []
        for u in users:
            holdings = db.get_holdings(u["discord_id"], SEASON)
            eval_total = u["cash"]
            for h in holdings:
                price_data = toss_api.get_price(h["symbol"])
                cur = float(price_data.get("lastPrice", h["avg_price"])) if price_data else h["avg_price"]
            ...
            scores.append((u["username"], pnl, pct, u["discord_id"] == str(interaction.user.id)))

        scores.sort(key=lambda x: x[1], reverse=True)

        medals = ["🥇", "🥈", "🥉"]
        embed = discord.Embed(title="🏆 시즌 1 수익률 랭킹", color=discord.Color.gold())
        for i, (name, pnl, pct, is_me) in enumerate(scores):
            medal = medals[i] if i < 3 else f"`{i+1}.`"
            me_tag = " ← 나" if is_me else ""
            sign = "▲" if pnl >= 0 else "▼"
            embed.add_field(
                name=f"{medal} {name}{me_tag}",
                value=f"{sign} {fmt(abs(pnl))}원 ({pct:+.2f}%)",
                inline=False,
            )
        embed.set_footer(text="3분마다 시세 반영 · 기준 자본금 1,000,000원")
        await interaction.followup.send(embed=embed)
